...
class PrestashopExportCategory(models.TransientModel):
    ...
    _description = "export all cateogries to ps"
    ...
    def export_categories(self):
        ...
        for category in category_obj.browse(self.env.context['active_ids']):
            ...
            if not ps_category:
                ...
                if ps_category:
                    ps_category.write({'default_shop_id': self.shop_id.id, })

            if not ps_category:
                ps_category = ps_category_obj.create({
                    'backend_id': self.backend_id.id,
                    'default_shop_id': self.shop_id.id,
                    'link_rewrite': self.backend_id.get_slug(category.name),
                    'odoo_id': category.id,
                })
            ps_category.with_context(connector_delay=True).re_export()
