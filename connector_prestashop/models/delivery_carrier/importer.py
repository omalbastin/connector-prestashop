...
class CarrierImportMapper(Component):
    ...
    direct = [
        ('name', 'name_ext'),
        ('name', 'name'),
        ('name', 'carrier_name'),  # to show the carrier name
...
class DeliveryCarrierBatchImporter(Component):
    ...
    """
    _name = 'prestashop.delivery.carrier.batch.importer'
...
class DeliveryCarrierMatchImporter(Component):
    """ match Import the PrestaShop Carriers.
    """
    _name = 'prestashop.delivery.carrier.match.importer'
    _inherit = 'prestashop.match.importer'
    _apply_on = 'prestashop.delivery.carrier'

    _erp_field = 'name'  # prestashop_bind_ids
    _ps_field = 'id_reference'
    _update_ps_id = True

    def compare_function(self, ps_val, erp_val, ps_dict, erp_dict):
        if ps_val and erp_val:
            return True
        return super().compare_function(ps_val, erp_val, ps_dict, erp_dict)

    def _odoo_domain_to_consider(self, ps_dict):
        res = super()._odoo_domain_to_consider(
            ps_dict)
        id_reference = int(str(ps_dict['id_reference']))
        ps_delivery = self.env['prestashop.delivery.carrier'].search([
            ('id_reference', '=', id_reference),
            ('backend_id', '=', self.backend_record.id)], order="id desc", limit=1)

        res += [('id', '=', ps_delivery.odoo_id.id)]
        return res


class DeliveryCarrierMatchBatchImporter(Component):
    """ match Import the PrestaShop Carriers.
    """
    _name = 'prestashop.delivery.carrier.match.batch.importer'
