...
class PrestashopBaseExporter(AbstractComponent):
    ...
    _usage = "prestashop.exporter"

    _base_mapper_usage = "prestashop.export.mapper"
    ...
    def run(self, binding, *args, **kwargs):
        ...
        """
        _logger.debug("Inside %s run, %s, binding:%s,args: %s,kwargs: %s" % (self._name, self, binding, args, kwargs))
    ...
    def _before_export(self):
        """if there is anything to be done before export."""
        return
...
class PrestashopExporter(AbstractComponent):
        ...
        try:
            ...
            if err.pgcode == psycopg2.errorcodes.UNIQUE_VIOLATION:
                raise RetryableJobError(
                    ...
                    f"{err}\n\n"
                    f"Likely due to 2 concurrent jobs wanting to create "
                    f"the same record. The job will be retried later."
                ) from err
            else:
                raise err from err
    ...
    def _export_dependency(
            ...
            relation,
            binding_model,
            component_usage="prestashop.exporter",
            binding_field_name="prestashop_bind_ids",
            bind_values=None,
            force_sync=False):
        """
        ...
        :param component_usage: 'usage' to look for to find the Component to
                                for the export, by default 'prestashop.exporter'
    ...
    def _lock(self):
        ...
        try:
            ...
            raise RetryableJobError(
                ...
                "({} with id {}). The job will be retried later.".format(
    ...
    def _run(self, fields=None, **kwargs):
        ...
        skip = self._has_to_skip(self.binding)
        if skip:
            return skip
...
class TranslationPrestashopExporter(AbstractComponent):
    ...
    #     _base_mapper_usage = 'prestashop.export.mapper.translate'

    def _get_record_by_odoo_lang_code(self, record):  # used in link_rewrite updation
        # get the backend's languages
        languages = self.backend_record.language_ids  # TODO this should be based on active lang
        records = {}
        # for each languages:
        for language in languages:
            # get the translated record
            lrecord = record.with_context(
                lang=language['code'])
            # put it in the dict
            records[language['code']] = lrecord
        return records
...
class PrestashopBatchExporter(AbstractComponent):
    ...
    _name = 'prestashop.batch.exporter'
    _inherit = ['base.exporter', 'base.prestashop.connector']
    _usage = 'prestashop.batch.exporter'
    ...
    def run(self, filters=None, **kwargs):
        ...
        if not filters:
            filters = []
        _logger.debug("Inside PrestashopBatchExporter %s run, %s, %s" % (self._name, self, kwargs))
        odoo_records = self.env[self.model.odoo_id._name].search(filters)
        for record in odoo_records:
            ...
            self._export_record(binding, **kwargs)

    def _export_record(self, binding, **kwargs):
        ...
        if self._use_job_queue:
            ...
            channel = kwargs.pop("channel", "root.prestashop_export")
            identity_key = kwargs.pop("identity_key", None)

            binding_obj = binding.with_delay(
                priority=priority,
                eta=eta,
                max_retries=max_retries,
                description=f"Export {binding.odoo_id._name} with ID {binding.odoo_id.id}({binding.odoo_id.display_name})",
                channel=channel,
                identity_key=identity_key,
            )
        binding_obj.export_record()
