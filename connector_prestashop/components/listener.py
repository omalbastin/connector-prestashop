...
class PrestashopListener(AbstractComponent):
    ...
    def need_to_export(self, record, fields=None):
        ...
        if not record or not record.backend_id:
            _logger.debug(
                "Skip export: No record or backend_id found", record)
            return True

        with record.backend_id.work_on(record._name) as work:
            mapper = work.component(usage='prestashop.export.mapper')
            ...
            if fields:
                if not exported_fields & set(fields):
                    ...
                    return True
        if record.no_export:
            _logger.debug("Skip export of %s because export is disable for it",
                          record)
            return True
        return False
