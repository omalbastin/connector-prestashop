...
class GenericAdapter(AbstractComponent):
    ...
    def create(self, attributes=None):
        """ Create a record on the external system """
        if not self.backend_record.export_active:
            raise exceptions.ValidationError(
                _(f"Create to PS Failed: Export is inactive for {self.backend_record.name}")
            )
        ...
        if not self._export_node_name_res:
            raise exceptions.ValidationError(
                    _(f"export_node_name_res not set for adapter {self._prestashop_model}"))
    ...
    def write(self, id_, attributes=None):
        ...
        if not self.backend_record.export_active:
            raise exceptions.ValidationError(
                _(f"Update to PS Failed: Export is inactive for {self.backend_record.name}")
            )
        attributes["id"] = id_
        _logger.debug(
